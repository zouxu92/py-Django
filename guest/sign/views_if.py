...
def add_event(request):
	...
	return JsonResponse({'status':200, 'message':'add enent success'})

# 发布会查询接口
def get_evnet_list(request):
...
def add_guest(request):
	...
	try:
		Guest.objects.creats(realname=realname,phone=int(phone),email=email,
...
def user_sign(request):
	...
	result = Event.objects.filter(id=eid)
	if not result:
		return JsonResponse({'status':10022,'message':'event id mull'})
